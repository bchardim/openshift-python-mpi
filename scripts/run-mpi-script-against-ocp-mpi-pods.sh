...
echo "#######################################################"
echo ""
echo "oc rsh --request-timeout=3600 ${mpi_pod_head} mpirun -np ${mpi_pods_count} -H ${mpi_host_list} \"bash -c '${mpi_scripts_dir}/$1 $2 $3'\""
oc rsh --request-timeout=3600 ${mpi_pod_head} mpirun -np ${mpi_pods_count} -H ${mpi_host_list} "bash -c '${mpi_scripts_dir}/$1 $2 $3'"
